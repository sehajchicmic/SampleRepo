...
public class VillageShop : MonoBehaviour
{
    ...
    public void onClickUpgrade(int itemIndex, GameObject item)
    {
        ...
        //if (villageManager.itemsUpgradeLevel[itemIndex] >= villageManager.currentVillage.items[itemIndex].upgradeLevel.Length)
        //{
        ...
        //Debug.Log(itemIndex);
        villageUpgrade.UpgradeItem(villageManager.currentVillage.items[itemIndex].upgradeLevel[villageManager.itemsUpgradeLevel[itemIndex]].upgradeItem , itemIndex);
        CloseShop();
    }
    ...
    private void onClick(GameObject item, int itemIndex)
    {
        ...
        item.GetComponent<ItemReference>().itemImageCurrentLevel.sprite = villageManager.currentVillage.items[itemIndex].upgradeLevel[villageManager.itemsUpgradeLevel[itemIndex]].destroyLevel[villageManager.currentDestroyLevel[itemIndex] - 1].destroyItem;
        if (villageManager.currentDestroyLevel[itemIndex] <= 1)
        {
            item.GetComponent<ItemReference>().itemImageNextLevel.enabled = false;
        }
        else
        {
            item.GetComponent<ItemReference>().itemImageNextLevel.sprite = villageManager.currentVillage.items[itemIndex].upgradeLevel[currentUpgradeLevel].destroyLevel[villageManager.currentDestroyLevel[itemIndex] - 2].destroyItem;
        }
        villageUpgrade.UpgradeItem(villageManager.currentVillage.items[itemIndex].upgradeLevel[currentUpgradeLevel].destroyLevel[villageManager.currentDestroyLevel[itemIndex] - 1].destroyItem, itemIndex);
        villageManager.currentDestroyLevel[itemIndex] -= 1;
        if (villageManager.currentDestroyLevel[itemIndex] <= 0)
        {
            item.GetComponent<ItemReference>().updateButton.enabled = false;
        }
        Debug.Log($"Current Destroy Level:-{villageManager.currentDestroyLevel[itemIndex] + 1}");
        CloseShop();
    }
    #endregion
